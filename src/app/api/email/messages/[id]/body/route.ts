...
function extractBodyFromParts(parts: GmailPart[]): string {
  ...
  for (const p of parts) {
    ...
    if (p.mimeType === "text/plain" && p.body?.data && !plainFallback) {
      plainFallback = Buffer.from(p.body.data, "base64").toString("utf-8");
    }
    if (p.parts?.length) {
  ...
  try {
    ...
    if (acc.provider === "gmail") {
      ...
      if (payload?.body?.data) {
        ...
        body = extractBodyFromParts(payload.parts as GmailPart[]);
      }
    ...
    if (acc.provider === "icloud") {
      ...
      try {
        ...
        if (!isNaN(uid)) {
          ...
          if (m && typeof m === "object" && "source" in m && m.source) {
            body = Buffer.isBuffer(m.source)
              ? (m.source as Buffer).toString("utf-8")
              : String(m.source);
          }
        }
      } finally {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to load email body" },
      { status: 500 }
    );
  }
}
